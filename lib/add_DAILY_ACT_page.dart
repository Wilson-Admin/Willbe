...
class ADD_DAILY_ACT_page extends StatefulWidget {

  String type = "";
  DateTime dateTime = DateTime.now();
  ADD_DAILY_ACT_page({String type="",DateTime? dateTime}){
    this.type = type;
    this.dateTime = dateTime!;
  }

  @override
  State<ADD_DAILY_ACT_page> createState() => ADD_DAILY_ACT_pageState(type:this.type,dateTime:this.dateTime);
}

class ADD_DAILY_ACT_pageState extends State<ADD_DAILY_ACT_page> {
  ...
  String type = "";
  ADD_DAILY_ACT_pageState({String type="",DateTime? dateTime}){
    this.type = type;
    this.dateTime = dateTime!;
  }
  ...
  Future<void> add_all_DAILY_MT_db_sub2()async{
    ...
    String MARK = "${sel_DAILY_ACT_ITEM.ITEM_NM}:${NOTE_textEditingController.text}";
...
DECLARE @Note NVARCHAR(MAX) = N'${NOTE_textEditingController.text}';   -- 備註內容
DECLARE @Date DATE = '${datetime}';
DECLARE @Type CHAR(4) = '${TYPE}';
DECLARE @ITEM CHAR(2) = '${sel_DAILY_ACT_ITEM.ITEM_NO}';
...
-- 8. 寫入 DAILY_ACT（備註）
INSERT INTO DAILY_ACT (TYPE, NO,ITEM, NOTE)
SELECT @Type, NO,@ITEM, @Note
FROM #NewNOs;
  ...
  Future<void> add_all_DAILY_MT_db_sub()async{
    ...
    for(int i=0;i<_cUSTOMERs.length;i++){
      ...
      if(View_DAILY_NO_num==-1){
        SmartDialog.dismiss();
        SmartDialog.showToast("請重新嘗試，或洽管理員");
        return;
      }
      ...
      dev.log("DAILY_PIC_DLs.length:${DAILY_PIC_DLs.length}");
...
FOR JSON PATH, WITHOUT_ARRAY_WRAPPER;
  ...
  Future<void> insert_DAILY_ACT_db_sub(
      {
        String TYPE="",//單別
        String NO="",//編號
        String ITEM="",//項目
    ...
    String comm = "INSERT INTO DAILY_ACT(TYPE,NO,ITEM,NOTE) VALUES ('${TYPE}','${NO}','${ITEM}','${NOTE}')";
    dev.log("${comm}");
        ...
        child:Scaffold(
          ...
          appBar: CustomAppBar(
            ...
            leading: GestureDetector(
                ...
                title: Text("新增活動", style: TextStyle(fontFamily: "GenJyuuGothic",fontWeight: FontWeight.bold,color: Color(0xff292929) , fontSize: 20.sp)),
              ),
          body: ListView(
            ...
            children: [
              ...
              Container(margin: EdgeInsets.only(left:8.w,right: 8.w,top: 0.h,bottom: 0.h),color: Color(0xffEEEEEE),width:ScreenUtil().screenWidth,height:50.h,child: DropdownButtonHideUnderline(
                child: DropdownButton2<DAILY_ACT_ITEM>(
                  isExpanded: true,
                  items: DAILY_ACT_ITEMs
                      .map((DAILY_ACT_ITEM item) => DropdownMenuItem<DAILY_ACT_ITEM>(
                    value: item,
                    child: Text(
                      item.ITEM_NM,
                      style: TextStyle(
                      ...
                      .toList(),
                  value: sel_DAILY_ACT_ITEM,
                  onChanged: (value) {

                    sel_DAILY_ACT_ITEM = value!;
                    setState(() {
                                ...
                                if( status.isGranted == false ) {
                                  ...
                                  if( status.isGranted == true) {
                                    ...
                                    for(int i=0;i<listImagePaths.length;i++){
                                      ...
                                      var decodedImage = File(listImagePaths[i].path!);
                                      dev.log("圖片大小(前):${decodedImage.length}");
                                      ...
                                      var decodedImage2 = File(filePath);
                                      dev.log("圖片大小(後):${decodedImage2.length}");
                                  ...
                                  for(int i=0;i<listImagePaths.length;i++){
                                    ...
                                    var decodedImage = File(listImagePaths[i].path!);
                                    dev.log("圖片大小(前):${decodedImage.readAsBytesSync().length}");
                                    ...
                                    var decodedImage2 = File(filePath);
                                    dev.log("圖片大小(後):${decodedImage2.readAsBytesSync().length}");
              ...
              Row(children: [
                    ...
                    child: PopupMenuButton<String>(
                        onSelected: (value) {
                          dev.log('你選了 $value');
                          NOTE_textEditingController.text = value;
                          setState(() {

                          });
                        },
                        itemBuilder: (BuildContext context) => sel_teacher_Daily_language_menu.menu[0].contants.map((e)=>PopupMenuItem<String>(
                        value: '${e}',
                      ...
                      style: ButtonStyle(
                          ...
                          onPressed: (sel_teacher_Daily_language_menu.menu[0].contants.isEmpty)?(){
