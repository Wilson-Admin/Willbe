...
class ADD_DAILY_TMP_page extends StatefulWidget {

  String type = "";
  DateTime dateTime = DateTime.now();
  ADD_DAILY_TMP_page({String type="",DateTime? dateTime}){
    this.type = type;
    this.dateTime = dateTime!;
  }

  @override
  State<ADD_DAILY_TMP_page> createState() => ADD_DAILY_TMP_pageState(type:this.type,dateTime:this.dateTime);
}

class ADD_DAILY_TMP_pageState extends State<ADD_DAILY_TMP_page> {
  ...
  String type = "";
  ADD_DAILY_TMP_pageState({String type="",DateTime? dateTime}){
    this.type = type;
    this.dateTime = dateTime!;
  }
...
DECLARE @TEMP DECIMAL(4,1) = CAST('${TEMP_textEditingController.text}' AS DECIMAL(4,1));
DECLARE @Date DATE = '${datetime}';
DECLARE @Type CHAR(4) = '${TYPE}';
DECLARE @KIND CHAR(1) = '${sel_DAILY_TMP_KIND_ITEM.ITEM_NO}';
...
-- 8. 寫入 DAILY_TMP（備註）
INSERT INTO DAILY_TMP (TYPE, NO, TEMP,KIND)
SELECT @Type, NO,@TEMP,@KIND
FROM #NewNOs;
...
FOR JSON PATH, WITHOUT_ARRAY_WRAPPER;
  ...
  Future<void> add_all_DAILY_MT_db_sub()async{
    ...
    for(int i=0;i<_cUSTOMERs.length;i++){
      ...
      int View_DAILY_NO_num = await read_DAILY_MT_db_sub();
...
FOR JSON PATH, WITHOUT_ARRAY_WRAPPER;
   ...
   */
  Future<void> insert_DAILY_TMP_db_sub(
      {
        String TYPE="",//單別
        String NO="",//編號
        String KIND="",//種類
    ...
    String comm = "INSERT INTO DAILY_TMP(TYPE,NO,KIND,TEMP) VALUES ('${TYPE}','${NO}','${KIND}','${TEMP}')";
    dev.log("${comm}");
  ...
  生活概況
   */
  Future<int> read_DAILY_MT_db_sub()async{

    int View_DAILY_NO_num=0;
    String datetime = "${DateFormat('yyyy-MM-dd').format(dateTime!)}";
    //String comm = "SELECT * FROM DAILY_MT WHERE TYPE='TMP' AND DATE BETWEEN '${datetime} 00:00:00' AND '${datetime} 23:59:59'";
    String comm = '''SELECT *
    FROM DAILY_MT
    WHERE TYPE = 'TMP'
        ...
        child:Scaffold(
          ...
          appBar: CustomAppBar(
            ...
            leading: GestureDetector(
                ...
                actions: [
                  GestureDetector(
                      onTap: (){
                        ...
                        if(EMPLOYEE_teacher.Teacher_CUSTOMER_selectedValue.cLASS.TYPE=="1" && timeOfDay==null){
                          SmartDialog.showToast('請先輸入時間');
                          return;
                        }

                        if(TEMP_textEditingController.text.isEmpty){
                          SmartDialog.showToast('請先輸入溫度');
                          return;
                        }
                ...
                title: Text("新增${DAILY_MT_TYPE_ITEMs.firstWhere((e)=>e.ITEM_NO=="TMP").ITEM_NM}", style: TextStyle(fontFamily: "GenJyuuGothic",fontWeight: FontWeight.bold,color: Color(0xff292929) , fontSize: 20.sp)),
              ),
          body: ListView(
            shrinkWrap: true,
            ...
            children: [
              ...
              Container(margin: EdgeInsets.only(left:8.w,right: 8.w,top: 0.h,bottom: 0.h),color: Color(0xffEEEEEE),width:ScreenUtil().screenWidth,height:50.h,child: DropdownButtonHideUnderline(
                child: DropdownButton2<DAILY_TMP_KIND_ITEM>(
                  isExpanded: true,
