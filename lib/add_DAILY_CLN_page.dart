...
class ADD_DAILY_CLN_page extends StatefulWidget {

  String type = "";
  DateTime? dateTime = DateTime.now();
  ADD_DAILY_CLN_page({String type="",DateTime? dateTime}){
    this.type = type;
    this.dateTime = dateTime!;
  }

  @override
  State<ADD_DAILY_CLN_page> createState() => ADD_DAILY_CLN_pageState(type:this.type,dateTime:this.dateTime);
}

class ADD_DAILY_CLN_pageState extends State<ADD_DAILY_CLN_page> {
  ...
  String type = "";
  ADD_DAILY_CLN_pageState({String type="",DateTime? dateTime}){
    this.type = type;
    this.dateTime = dateTime!;
  }
...
DECLARE @NOTE NVARCHAR(255) = N'${NOTE_textEditingController.text}';   -- 備註內容
DECLARE @Date DATE = '${datetime}';
DECLARE @Type CHAR(4) = '${TYPE}';
DECLARE @HAIR BIT = '${is_HAIR!?"1":"0"}';
...
-- 8. 寫入 DAILY_CLN（備註）
INSERT INTO DAILY_CLN (TYPE, NO,HAIR, NOTE)
SELECT @Type, NO,@HAIR, @NOTE
FROM #NewNOs;
...
FOR JSON PATH, WITHOUT_ARRAY_WRAPPER;
   ...
   */
  Future<void> insert_DAILY_CLN_db_sub(
      {
        String TYPE="",//單別
        String NO="",//編號
        bool? HAIR,//洗頭
    ...
    String comm = "INSERT INTO DAILY_CLN(TYPE,NO,HAIR,NOTE) VALUES ('${TYPE}','${NO}','${HAIR}','${NOTE}')";
    dev.log("${comm}");
  ...
  Future<int> read_View_DAILY_db_sub()async{
    ...
    String datetime = "${DateFormat('yyyy-MM-dd').format(dateTime!)}";
    //String comm = "SELECT * FROM DAILY_MT WHERE TYPE='CLN' AND DATE BETWEEN '${datetime} 00:00:00' AND '${datetime} 23:59:59'";
    String comm = '''SELECT *
    FROM DAILY_MT
    WHERE TYPE = 'CLN'
        ...
        child:Scaffold(
          ...
          appBar: CustomAppBar(
            ...
            leading: GestureDetector(
                ...
                title: Text("新增${DAILY_MT_TYPE_ITEMs.firstWhere((e)=>e.ITEM_NO=="CLN").ITEM_NM}", style: TextStyle(fontFamily: "GenJyuuGothic",fontWeight: FontWeight.bold,color: Color(0xff292929) , fontSize: 20.sp)),
              ),
          body: ListView(
            shrinkWrap: true,
