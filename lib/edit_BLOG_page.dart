...
class EDIT_BLOG_page extends StatefulWidget {

  BLOG bLOG = BLOG();
  EDIT_BLOG_page({BLOG? bLOG}){
    this.bLOG = bLOG!;
  }

  @override
  State<EDIT_BLOG_page> createState() => EDIT_BLOG_pageState(bLOG:this.bLOG);
}

class EDIT_BLOG_pageState extends State<EDIT_BLOG_page> {
  ...
  void initState() {
    ...
    TITLE_textEditingController.text = "${bLOG.TITLE}";
    DETAIL_textEditingController.text = "${bLOG.DETAIL}";
    NOTE_textEditingController.text = "${bLOG.NOTE}";
    dev.log("bLOG.BLTN_DT:${bLOG.BLTN_DT}");
    if("${bLOG.BLTN_DT}".isNotEmpty){
      dateTime = DateTime.parse("${bLOG.BLTN_DT}");
    }

    bLOG_DL.addAll(bLOG.bLOG_DL);
    bLOG_DL.sort((a,b)=> a.BLOG_SR.compareTo(b.BLOG_SR));

    if(bLOG_DL.length>0){
      dev.log("BLOG_SR max:${bLOG_DL[bLOG_DL.length-1].BLOG_SR}");
      BLOG_SR_count = int.parse(bLOG_DL[bLOG_DL.length-1].BLOG_SR);
    }
  ...
  上傳新一筆活動花絮
   */
  Future<void> upload_BLOG_db_sub()async{
    FocusManager.instance.primaryFocus?.unfocus();
    SmartDialog.showLoading(msg: "處理中...");
    await Future.delayed(const Duration(milliseconds: 500), () {});

    await updata_BLOG_db_sub(
        BLOG_NO:bLOG.BLOG_NO,//生活花絮編號
    ...
    await delete_BLOG_DL_db_sub(BLOG_NO:bLOG.BLOG_NO);

    //更新生活花絮子表單
    for(int i=0;i<bLOG_DL.length;i++){

      BLOG_SR_count+=1;
      String img_name = "${BLOG_SR_count}_${user.ACCOUNT}_${DateTime.now().microsecondsSinceEpoch}";
      if(bLOG_DL[i].prescriptionsbytes_xfile!=null){
        ...
        await insert_BLOG_DL_db_sub(
            BLOG_NO:bLOG.BLOG_NO,//生活花絮編號
            BLOG_SR:"${BLOG_SR_count}",
            LINK:"~/School/Images/Blog/${img_name}.jpg",//
            NOTE:"${bLOG_DL[i].NOTE_textEditingController.text}"
        );
      }
      else if(bLOG_DL[i].LINK.isNotEmpty){
        http.Response response = await http.get(Uri.parse(bLOG_DL[i].LINK));//Uint8List
        await upload_image(img: response.bodyBytes,file_name: img_name,folder: "Blog");
        await insert_BLOG_DL_db_sub(
            BLOG_NO:bLOG.BLOG_NO,//生活花絮編號
            BLOG_SR:"${BLOG_SR_count}",
            LINK:"~/School/Images/Blog/${img_name}.jpg",//
  ...
  生活花絮
   */
  Future<void> updata_BLOG_db_sub(
      {
    ...
    String comm = "UPDATE BLOG SET TITLE='${TITLE}',DETAIL='${DETAIL}',BLTN_DT='${BLTN_DT}',NOTE='${NOTE}' WHERE BLOG_NO='${BLOG_NO}'";
    dev.log("${comm}");
    ...
    return GestureDetector(
        ...
        child:Scaffold(
          ...
          appBar: CustomAppBar(
            ...
            title: Text("編輯活動花絮", style: TextStyle(fontFamily: "GenJyuuGothic",fontWeight: FontWeight.bold,color: Color(0xff292929) , fontSize: 20.sp)),
          ),
      body: ListView(
        shrinkWrap: true,
        ...
        children: [
          ...
          GestureDetector(
              onTap:()async{
                ...
                //2026/05/07 , 禁止使用者變更日期 , 導致新增花絮會出現序號重複問題
                /*
