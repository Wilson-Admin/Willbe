...
class ADD_DAILY_EAT_page extends StatefulWidget {

  String type = "";
  DateTime dateTime = DateTime.now();
  ADD_DAILY_EAT_page({String type="",DateTime? dateTime}){
    this.type = type;
    this.dateTime = dateTime!;
  }

  @override
  State<ADD_DAILY_EAT_page> createState() => ADD_DAILY_EAT_pageState(type:this.type,dateTime:this.dateTime);
}

class ADD_DAILY_EAT_pageState extends State<ADD_DAILY_EAT_page> {
  ...
  List<DAILY_PIC_DL> DAILY_PIC_DLs = [];//

  ScrollController listScrollController = ScrollController();

  String type = "";
  ADD_DAILY_EAT_pageState({String type="",DateTime? dateTime}){
    this.type = type;
    this.dateTime = dateTime!;
  }
...
DECLARE @SPECIAL NVARCHAR(255) = N'${SPECIAL_textEditingController.text}';   -- 備註內容
DECLARE @Date DATE = '${datetime}';
DECLARE @Type CHAR(4) = '${TYPE}';
DECLARE @ITEM CHAR(2) = '${sel_DAILY_EAT_ITEM.ITEM_NO}';
DECLARE @NOTE CHAR(2) = '${sel_DAILY_EAT_NOTE_ITEM.ITEM_NO}';
...
-- 8. 寫入 DAILY_EAT（備註）
INSERT INTO DAILY_EAT (TYPE, NO,ITEM, NOTE,OTHER,SPECIAL)
SELECT @Type, NO,@ITEM, @NOTE,@OTHER,@SPECIAL
FROM #NewNOs;
...
FOR JSON PATH, WITHOUT_ARRAY_WRAPPER;
   ...
   */
  Future<void> insert_DAILY_EAT_db_sub(
      {
        String TYPE="",//單別
        String NO="",//編號
        String ITEM="",//項目
    ...
    String comm = "INSERT INTO DAILY_EAT(TYPE,NO,ITEM,NOTE,OTHER,SPECIAL) VALUES ('${TYPE}','${NO}','${ITEM}','${NOTE}','${OTHER}','${SPECIAL}')";
    dev.log("${comm}");
  ...
  Future<int> read_View_DAILY_db_sub()async{
    ...
    String datetime = "${DateFormat('yyyy-MM-dd').format(dateTime!)}";
    //String comm = "SELECT * FROM DAILY_MT WHERE TYPE='EAT' AND DATE BETWEEN '${datetime} 00:00:00' AND '${datetime} 23:59:59'";
    String comm = '''SELECT *
    FROM DAILY_MT
    WHERE TYPE = 'EAT'
        ...
        child:Scaffold(
          ...
          appBar: CustomAppBar(
            ...
            leading: GestureDetector(
                ...
                title: Text("新增${DAILY_MT_TYPE_ITEMs.firstWhere((e)=>e.ITEM_NO=="EAT").ITEM_NM}", style: TextStyle(fontFamily: "GenJyuuGothic",fontWeight: FontWeight.bold,color: Color(0xff292929) , fontSize: 20.sp)),
              ),
          body: ListView(
            ...
            children: [
              ...
              Container(margin: EdgeInsets.only(left:8.w,right: 8.w,top: 0.h,bottom: 0.h),color: Color(0xffEEEEEE),width:ScreenUtil().screenWidth,height:50.h,child: DropdownButtonHideUnderline(
                child: DropdownButton2<DAILY_EAT_ITEM>(
                  isExpanded: true,
              ...
              Container(margin: EdgeInsets.only(left:8.w,right: 8.w,top: 0.h,bottom: 0.h),color: Color(0xffEEEEEE),width:ScreenUtil().screenWidth,height:50.h,child: DropdownButtonHideUnderline(
                child: DropdownButton2<DAILY_EAT_NOTE_ITEM>(
                  isExpanded: true,
