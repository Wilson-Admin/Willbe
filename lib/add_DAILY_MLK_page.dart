...
class ADD_DAILY_MLK_page extends StatefulWidget {

  String type = "";
  DateTime dateTime = DateTime.now();
  ADD_DAILY_MLK_page({String type="",DateTime? dateTime}){
    this.type = type;
    this.dateTime = dateTime!;
  }

  @override
  State<ADD_DAILY_MLK_page> createState() => ADD_DAILY_MLK_pageState(type:this.type,dateTime:this.dateTime);
}

class ADD_DAILY_MLK_pageState extends State<ADD_DAILY_MLK_page> {
  ...
  String type = "";
  ADD_DAILY_MLK_pageState({String type="",DateTime? dateTime}){
    this.type = type;
    this.dateTime = dateTime!;
  }
  ...
  Future<void> add_all_DAILY_MT_db_sub2()async{
    ...
    String MARK = "${sel_DAILY_MLK_ITEM.ITEM_NM}:${CAPACITY_textEditingController.text} ml\n${NOTE_textEditingController.text}";
...
DECLARE @ITEM CHAR(1) = '${sel_DAILY_MLK_ITEM.ITEM_NO}';
DECLARE @CAPACITY CHAR(4) = N'${CAPACITY_textEditingController.text}';
...
SELECT
    ...
    @Date,
    CASE 
        WHEN '${EMPLOYEE_teacher.Teacher_CUSTOMER_selectedValue.cLASS.TYPE}' = '2' 
             THEN '${timeOfDay==null?"":formatTimeOfDay(timeOfDay!)}'
        ELSE CAST(SYSDATETIME() AS TIME)  -- 不要加引號
    END,
    c.DEPM_NO,
    c.CLASS_NO,
    c.CS_NO,
...
-- 8. 寫入 DAILY_MLK（備註）
INSERT INTO DAILY_MLK (TYPE, NO,ITEM, CAPACITY,NOTE)
SELECT @Type, NO,@ITEM, @CAPACITY,@NOTE
FROM #NewNOs;
...
FOR JSON PATH, WITHOUT_ARRAY_WRAPPER;
   ...
   */
  Future<void> insert_DAILY_MLK_db_sub(
      {
        String TYPE="",//單別
        String NO="",//編號
        String ITEM="",//項目
    ...
    String comm = "INSERT INTO DAILY_MLK(TYPE,NO,ITEM,CAPACITY) VALUES ('${TYPE}','${NO}','${ITEM}','${CAPACITY}')";
    dev.log("${comm}");
  ...
  Future<int> read_View_DAILY_db_sub()async{
    ...
    String datetime = "${DateFormat('yyyy-MM-dd').format(dateTime)}";
    //String comm = "SELECT * FROM DAILY_MT WHERE TYPE='MLK' AND DATE BETWEEN '${datetime} 00:00:00' AND '${datetime} 23:59:59'";
    String comm = '''SELECT *
    FROM DAILY_MT
    WHERE TYPE = 'MLK'
          ...
          appBar: CustomAppBar(
            ...
            leading: GestureDetector(
                onTap: (){
                  ...
                  if(CAPACITY_textEditingController.text.isNotEmpty){
                    check=true;
                  }
                ...
                actions: [
                  GestureDetector(
                      onTap: (){
                        ...
                        if(CAPACITY_textEditingController.text.isEmpty){
                          SmartDialog.showToast("請輸入餵奶ml");
                          return;
                        }

                        if(EMPLOYEE_teacher.Teacher_CUSTOMER_selectedValue.cLASS.TYPE=="2" && timeOfDay==null){
                          SmartDialog.showToast('請先輸入時間');
                          return;
                        }
                ...
                title: Text("新增${DAILY_MT_TYPE_ITEMs.firstWhere((e)=>e.ITEM_NO=="MLK").ITEM_NM}", style: TextStyle(fontFamily: "GenJyuuGothic",fontWeight: FontWeight.bold,color: Color(0xff292929) , fontSize: 20.sp)),
              ),
          body: ListView(
            shrinkWrap: true,
            ...
            children: [
              ...
              Container(margin: EdgeInsets.only(left:8.w,right: 8.w,top: 0.h,bottom: 0.h),color: Color(0xffEEEEEE),width:ScreenUtil().screenWidth,height:50.h,child: DropdownButtonHideUnderline(
                child: DropdownButton2<DAILY_MLK_ITEM>(
                  isExpanded: true,
              ...
              //建立體溫，可選擇時間(幼兒園)
              (EMPLOYEE_teacher.Teacher_CUSTOMER_selectedValue.cLASS.TYPE=="2")?
              Column(children: [
                Container(height: 15.h,),
                Row(children: [
                  Container(width: 10.w,),
