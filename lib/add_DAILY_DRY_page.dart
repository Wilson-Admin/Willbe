...
class ADD_DAILY_DRY_page extends StatefulWidget {

  String type = "";
  DateTime dateTime = DateTime.now();
  ADD_DAILY_DRY_page({String type="",DateTime? dateTime}){
    this.type = type;
    this.dateTime = dateTime!;
  }

  @override
  State<ADD_DAILY_DRY_page> createState() => ADD_DAILY_DRY_pageState(type:this.type,dateTime:this.dateTime);
}

class ADD_DAILY_DRY_pageState extends State<ADD_DAILY_DRY_page> {
  ...
  String type = "";
  ADD_DAILY_DRY_pageState({String type="",DateTime? dateTime}){
    this.type = type;
    this.dateTime = dateTime!;
  }
  ...
  Future<void> add_all_DAILY_MT_db_sub2()async{
    ...
    String MARK = "${NOTE_textEditingController.text}";
...
-- 8. 寫入 DAILY_DRY（備註）
INSERT INTO DAILY_DRY (TYPE, NO, NOTE)
SELECT @Type, NO, @Note
FROM #NewNOs;
...
FOR JSON PATH, WITHOUT_ARRAY_WRAPPER;
    ...
    dev.log("全班寫入日記(回應):${result}");

    //response.body:{"message":"執行成功","affectedRows":15,"FirstNO":"2507210000008"}
    ...
    try{
      ...
      if("${map["message"]}"=="執行成功"){
        ...
        //Student_T_page_fun!(action:"新增[托嬰/幼兒]日記 副表成功");
        DAILY_PIC_DLs.clear();
...
FOR JSON PATH, WITHOUT_ARRAY_WRAPPER;
    ...
    try{
      ...
      if("${map["message"]}"=="執行成功"){
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              title: Text('⚠️ 警告',textScaler: TextScaler.linear(1),style: TextStyle(color: Colors.black,fontSize: 20.sp),),
              content: Text('上傳失敗，請重試(1)\n${result}',textScaler: TextScaler.linear(1),style: TextStyle(color: Colors.black,fontSize: 20.sp),),
              actions: <Widget>[
                TextButton(
                  child:  Text('關閉',textScaler: TextScaler.linear(1),style: TextStyle(color: Colors.black,fontSize: 20.sp),),
                  onPressed: () {
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text('⚠️ 警告',textScaler: TextScaler.linear(1),style: TextStyle(color: Colors.black,fontSize: 20.sp),),
            content: Text('上傳失敗，請重試(2)\n${e.toString()}',textScaler: TextScaler.linear(1),style: TextStyle(color: Colors.black,fontSize: 20.sp),),
            actions: <Widget>[
              TextButton(
                child:  Text('關閉',textScaler: TextScaler.linear(1),style: TextStyle(color: Colors.black,fontSize: 20.sp),),
                onPressed: () {
   ...
   */
  Future<void> insert_DAILY_DRY_db_sub(
      {
        String TYPE="",//單別
        String NO="",//編號
        String NOTE="",//內容
    ...
    String comm = "INSERT INTO DAILY_DRY(TYPE,NO,NOTE) VALUES ('${TYPE}','${NO}','${NOTE}')";
    dev.log("${comm}");
  ...
  Future<int> read_View_DAILY_db_sub()async{
    ...
    String datetime = "${DateFormat('yyyy-MM-dd').format(dateTime!)}";
    //String comm = "SELECT * FROM DAILY_MT WHERE TYPE='DRY' AND DATE BETWEEN '${datetime} 00:00:00' AND '${datetime} 23:59:59'";
    String comm = '''SELECT *
    FROM DAILY_MT
    WHERE TYPE = 'DRY'
        ...
        child:Scaffold(
          ...
          appBar: CustomAppBar(
            ...
            leading: GestureDetector(
                ...
                title: Text("新增${DAILY_MT_TYPE_ITEMs.firstWhere((e)=>e.ITEM_NO=="DRY").ITEM_NM}", style: TextStyle(fontFamily: "GenJyuuGothic",fontWeight: FontWeight.bold,color: Color(0xff292929) , fontSize: 20.sp)),
              ),
          body: ListView(
              ...
              Row(children: [
                    ...
                    Container(
                        ...
                        child: PopupMenuButton<String>(
                            ...
                            itemBuilder: (BuildContext context) => sel_teacher_Daily_language_menu.menu[2].contants.map((e)=>PopupMenuItem<String>(
                                value: '${e}',
                            ...
                            child: ElevatedButton(
                              ...
                              onPressed: (sel_teacher_Daily_language_menu.menu[2].contants.isEmpty)?(){
